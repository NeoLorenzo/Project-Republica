...
- Deterministic accounting/derived metrics (for example budget nodes, `government_expenditure`, `gdp_gov_consumption_G_eur_m`, `gdp`, `debt_to_gdp`, and population stock-flow nodes such as `births_annual` and `population.total`) are computed by arithmetic helpers, not behavioral edge propagation.
